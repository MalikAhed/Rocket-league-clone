...
export function encodeCarConfig(config) {
  ...
  bytes[96] = 0;
  const dodgeDeadzone=config.dodgeDeadzone??.5;
  if(!Number.isFinite(dodgeDeadzone)||dodgeDeadzone<=0||dodgeDeadzone>1)throw new RangeError('Invalid dodge deadzone');
  put(100, dodgeDeadzone);
  return bytes;
}
